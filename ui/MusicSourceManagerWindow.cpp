...
#include "MusicSourceSyncSettingsDialog.h"
...
#define B_TRANSLATION_CONTEXT "MusicSourceManagerWindow"

MusicSourceManagerWindow::MusicSourceManagerWindow(BMessenger cacheManager)
    : BWindow(BRect(100, 100, 500, 400), B_TRANSLATE("Manage Music Folders"),
              B_TITLED_WINDOW, B_ASYNCHRONOUS_CONTROLS),
      fMediaLibraryCache(cacheManager) {
  fDirectoryList = new BListView("directoryList");
  fDirectoryList->SetInvocationMessage(new BMessage(MSG_DIR_EDIT));
  fDirectoryList->SetSelectionMessage(new BMessage(MSG_SELECTION_CHANGED));
  BScrollView *scroll =
  ...
  fBtnSettings = new BButton("Settings", B_TRANSLATE("Settings"),
                             new BMessage(MSG_DIR_EDIT));
  fBtnRemove->SetEnabled(false);
  fBtnSettings->SetEnabled(false);
...
void MusicSourceManagerWindow::MessageReceived(BMessage *msg) {
  switch (msg->what) {
  case MSG_DIR_ADD:
    fAddPanel->Show();
    break;
  ...
  case MSG_SELECTION_CHANGED: {
    bool hasSelection = fDirectoryList->CurrentSelection() >= 0;
    fBtnRemove->SetEnabled(hasSelection);
    fBtnSettings->SetEnabled(hasSelection);
    break;
  }
  ...
  case MSG_DIR_OK:
    SaveSettings();
    if (fMediaLibraryCache.IsValid()) {
      fMediaLibraryCache.SendMessage(MSG_RESCAN);
    }
    Quit();
    break;
...
void MusicSourceManagerWindow::AddDirectory(const entry_ref &ref) {
  BPath path(&ref);
  ...
  bool isBfs = MetadataTagIO::IsBeFsVolume(path);

  MusicSourceSyncSettingsDialog *dialog =
      new MusicSourceSyncSettingsDialog(BMessenger(this), -1, path.Path(), isBfs);
  dialog->Show();
}

void MusicSourceManagerWindow::RemoveSelectedDirectory() {
  int32 index = fDirectoryList->CurrentSelection();
  if (index < 0)
    return;
...
void MusicSourceManagerWindow::EditDirectory(int32 index) {
  if (index < 0 || index >= static_cast<int32>(fSources.size()))
    return;

  MusicSourceSettings &src = fSources[index];
  BPath path(src.path.String());
  bool isBfs = MetadataTagIO::IsBeFsVolume(path);

  MusicSourceSyncSettingsDialog *dialog = new MusicSourceSyncSettingsDialog(
      BMessenger(this), index, src.path.String(), isBfs, &src);
  dialog->Show();
}

void MusicSourceManagerWindow::SaveSettings() {
  BPath settingsPath;
  ...
  MusicSourceSettings::InvalidateCache();
}

void MusicSourceManagerWindow::LoadSettings() {
  BPath settingsPath;
  if (find_directory(B_USER_SETTINGS_DIRECTORY, &settingsPath) != B_OK)
    return;
...
void MusicSourceManagerWindow::MigrateFromOldFormat() {
  BPath oldPath;
