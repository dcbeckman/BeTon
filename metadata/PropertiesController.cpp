  ...
  for (int32 i = 0; files.FindRef("refs", i, &ref) == B_OK; i++) {
    ...
    if (file.InitCheck() == B_OK) {
      ...
      DEBUG_PRINT("Set rating %ld for %s\n", (long)rating, path.Path());
        ...
        // Read tags for fallback and write updated rating to embedded tags
        TagData td;
        ...
        if (mapIt != fWindow->fPathIndex.end()) {
            ...
            mi.rating = rating;
            // Populate fields from cache entry
            ...
            // Use tag data as source when item not in cache (Folder view)
