...
bool MetadataTagIO::ReadTags(const BPath &path, TagData &out,
  ...
  if (lower.EndsWith(".mid") || lower.EndsWith(".midi")) {
    return ReadBfsAttributes(path, out);
  }
...
bool MetadataTagIO::WriteTagsToFile(const BPath &path, const TagData &td,
  ...
  if (access(path.Path(), W_OK) != 0) {
    DEBUG_PRINT("WriteTagsToFile: file is read-only: %s\n",
                path.Path());
    return false;
  }
  BString p = path.Path();
  ...
  if (lower.EndsWith(".mp3")) {
    ...
    if (id3) {
      ...
      if (coverOpt && coverOpt->data() && coverOpt->size() > 0) {
        DEBUG_PRINT("WriteTagsToFile: embedding MP3 cover, "
                    "size=%zu\n",
                    coverOpt->size());
        std::vector<TagLib::ID3v2::Frame *> toRemove;
        ...
        const char *mime = sniff_mime((const uint8 *)coverOpt->data(),
                                      coverOpt->size());
        ...
        pic->setPicture(TagLib::ByteVector(
            reinterpret_cast<const char *>(coverOpt->data()),
            static_cast<unsigned int>(coverOpt->size())));
        id3->addFrame(pic);
      }
    }
...
/**
 * @brief Writes metadata tags without modifying embedded cover art.
 */
...
/**
 * @brief Checks whether a file resides on a BFS volume.
 */
bool MetadataTagIO::IsBeFsVolume(const BPath &path) {
...
bool MetadataTagIO::WriteBfsAttributes(const BPath &path, const TagData &td,
  ...
  if (access(path.Path(), W_OK) != 0) {
    DEBUG_PRINT("WriteBfsAttributes: file is read-only: %s\n",
                path.Path());
    return false;
  }
  BNode n(&e);
  if (n.InitCheck() != B_OK) {
    DEBUG_PRINT("BNode init failed for %s\n", path.Path());
    return false;
  }
  ...
  DEBUG_PRINT("write attrs %s: %s\n", path.Path(), ok ? "OK" : "FAILED");
  return ok;
}

/**
 * @brief Writes or removes embedded cover art in a supported audio file.
 */
bool MetadataTagIO::WriteEmbeddedCover(const BPath &file, const uint8 *data,
  ...
  if (access(file.Path(), W_OK) != 0) {
    DEBUG_PRINT("WriteEmbeddedCover: file is read-only: %s\n",
                file.Path());
    return false;
  }
...
/**
 * @brief Convenience overload for writing embedded cover from `CoverBlob`.
 */
bool MetadataTagIO::WriteEmbeddedCover(const BPath &file, const CoverBlob &blob,
                                 const char *mimeOpt) {
...
/**
 * @brief Reads supported BFS metadata attributes into `TagData`.
 */
bool MetadataTagIO::ReadBfsAttributes(const BPath &path, TagData &out) {
  ...
  if (node.ReadAttr("Media:Length", B_INT32_TYPE, 0, &intVal,
                    sizeof(intVal)) > 0)
    out.lengthSec = static_cast<uint32>(intVal);

  if (node.ReadAttr("Audio:Bitrate", B_INT32_TYPE, 0, &intVal,
                    sizeof(intVal)) > 0)
    out.bitrate = static_cast<uint32>(intVal);
...
/**
 * @brief Merges metadata from two sources according to conflict mode.
 */
TagData MetadataTagIO::MergeMetadata(const TagData &primary, const TagData &secondary,
                               int32 mode) {
...
/**
 * @brief Applies synchronization output to tags or BFS attributes.
 */
bool MetadataTagIO::ApplySync(const BPath &path, const TagData &source,
                        bool towardsBfs) {
  if (towardsBfs) {
...
/**
 * @brief Merges with conflict detection while favoring non-empty fields.
 */
bool MetadataTagIO::SmartMerge(const TagData &primary, const TagData &secondary,
